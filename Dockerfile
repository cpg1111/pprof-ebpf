...
RUN apt-get update && apt install -y apt-transport-https ca-certificates software-properties-common && \
    ...
    apt-get update &&\
    apt-get install -y bcc-tools libbcc-examples linux-headers-4.14.22-86 golang git build-essential cmake llvm-3.8 libclang-3.8-dev\
    bison python zlib-dev libelf-dev flex libedit-dev && \
